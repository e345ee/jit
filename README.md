...
Если пользователь открывает mini app из MAX-бота, ссылка передает технический `maxTarget`, и worker отправляет due-напоминания через MAX Bot API. При прямом открытии web-интерфейса напоминание остается локально проверяемым и не содержит медицинских подробностей.
