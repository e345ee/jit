...
Если пользователь открывает mini app из MAX-бота, нативная кнопка `open_app` передает технический payload с целевым чатом или пользователем, а web-приложение валидирует подписанные `initData` через API. Прямой браузерный вход в production показывает экран открытия через MAX; worker отправляет due-напоминания через MAX Bot API.
