...
- Mini app в production не раскрывает основной интерфейс при прямом браузерном открытии: фронт отправляет MAX `initData` в `POST /max/session`, API проверяет HMAC-подпись через `MAX_BOT_TOKEN`, и только после этого приложение показывает маршруты.
